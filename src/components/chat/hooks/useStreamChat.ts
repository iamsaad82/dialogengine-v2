...
export function useStreamChat({ 
  ...
  const processStreamEvent = useCallback((event: MessageEvent) => {
    ...
    if (eventType === 'section' || (data && (data.type || data.event === 'token'))) {
      ...
      // Nach 300ms den Ladestatus zurücksetzen (erst nach dem Hinzufügen der Nachricht)
      setTimeout(() => {
        setIsLoading(false);
        console.log("STREAM-DEBUG-007: Ladestatus zurückgesetzt");
        
        // Nach weiteren 200ms den gestreamten Inhalt zurücksetzen
        setTimeout(() => {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (isLoading && (currentStreamedMessage || lastStreamContentRef.current)) {
        ...
        // Zurücksetzen des Stream-Status mit Verzögerung
        setTimeout(() => {
          setIsLoading(false);
  ...
  const addMessageAndResetStream = useCallback((message: Message, originalContent: string) => {
    ...
    // Verzögere das Zurücksetzen des Stream-Status deutlich
    // Wichtig: Ein längerer Timeout gibt dem UI mehr Zeit zur Aktualisierung
    setTimeout(() => {
      console.log("STREAM-DEBUG-005: Setze Stream-Status zurück nach 1000ms Verzögerung");
      setIsLoading(false);
      
      // Weitere 500ms Verzögerung vor dem Zurücksetzen des gestreamten Inhalts
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (isLoading && currentStreamedMessage) {
      ...
      }, 10000); // 10 Sekunden ohne Updates als Timeout betrachten
