...
// Parkgebühren-Datenstruktur
export interface ParkingData {
  fees: { duration: string; price: string }[];
  notes?: string[];
}

interface FluidParkingCardProps {
  title: string;
...
/**
 * Eine flüssige Parkgebühren-Karte mit optimiertem Streaming-Verhalten
 * 
 ...
 */
const FluidParkingCard: React.FC<FluidParkingCardProps> = ({
  title,
  data,
  isStreaming,
  colorStyle = {
  ...
  useEffect(() => {
    const mainTimer = setTimeout(() => {
      ...
      setTimeout(() => setFeesVisible(true), 100);
      setTimeout(() => setNotesVisible(true), 300);
    }, 100);
    
    return () => clearTimeout(mainTimer);
  }, []);
  
  // Erstelle Platzhalter für Parkgebühren
  const feesPlaceholders = Array(4).fill(null);
  ...
  return (
    <div style={cardStyle} className="fluid-parking-card">
      {/* Header */}
      ...
      <div style={contentStyle}>
        ...
        <div 
          style={{
            ...sectionStyle,
            opacity: feesVisible ? 1 : 0,
            transform: feesVisible ? 'translateY(0)' : 'translateY(10px)',
          }}
        >
          <table style={tableStyle}>
            <tbody>
              {data && data.fees ? (
                ...
                feesPlaceholders.map((_, index) => (
                  <tr key={`placeholder-fee-${index}`} style={rowStyle}>
                    <td style={{...cellStyle, ...durationStyle}}>
...
export default FluidParkingCard;
