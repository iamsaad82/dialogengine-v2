...
const FluidRestaurantSlider: React.FC<FluidRestaurantSliderProps> = ({
  ...
  const [visibleItems, setVisibleItems] = useState<ShopData[]>([]);
  
  // Memoized Werte für Stabilität
  const placeholders = useMemo(() => Array(maxItems).fill(null), [maxItems]);
  const cardWidth = useMemo(() => {
    return restaurants.length < 3 ? `calc((100% / ${Math.max(restaurants.length || 1, 1)}) - 1rem)` : '280px';
  }, [restaurants.length]);
  ...
  useEffect(() => {
    ...
    // Sanft neue Restaurants hinzufügen, mit Verzögerung für flüssige Animation
    const timer = setTimeout(() => {
      setVisibleItems(restaurants.slice(0, maxItems));
    }, 100);
    
    return () => clearTimeout(timer);
  }, [restaurants, maxItems, isInitialRender]);
  ...
  return (
    <div ref={sliderRef} style={sliderStyle} className="fluid-restaurant-slider">
      ...
      >
        {/* Zeige entweder die tatsächlichen Restaurants oder Platzhalter an */}
        {placeholders.map((_, index) => (
          <FluidRestaurantCard
            key={`restaurant-card-${index}`}
            restaurant={visibleItems[index] || null}
