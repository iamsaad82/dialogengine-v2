...
import { ServiceData } from './FluidServiceSlider';

interface FluidServiceCardProps {
...
/**
 * Eine flüssige Service-Karte mit optimiertem Streaming-Verhalten
 * 
 ...
 */
const FluidServiceCard: React.FC<FluidServiceCardProps> = ({
  service,
  isLoading,
  cardWidth,
  colorStyle,
  ...
  useEffect(() => {
    if (service?.image && imageRef.current) {
      const img = new Image();
  ...
  const cardStyle: React.CSSProperties = {
    width: cardWidth,
    minWidth: '250px',
    height: '320px', // Etwas kleiner als die Shop-Karten
    display: 'flex',
  ...
  // Styling für den Standort mit Skeleton-Loader
  const locationStyle: React.CSSProperties = {
    fontSize: '0.9rem',
    color: '#666',
    marginBottom: '0.5rem',
    height: '1.2rem',
    ...getSkeletonStyle('60%', '1.2rem', 150),
  };
  ...
  const descriptionStyle: React.CSSProperties = {
    ...
    marginBottom: '0.5rem',
    flex: 1,
    ...getSkeletonStyle('100%', 'auto', 200),
  };
  
  // Fortschrittlicher Skeleton-Loader für die Beschreibung mit Welleneffekt
  const DescriptionSkeleton = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      <div style={getSkeletonStyle('100%', '0.9rem', 200)} />
      <div style={getSkeletonStyle('90%', '0.9rem', 250)} />
      <div style={getSkeletonStyle('80%', '0.9rem', 300)} />
    </div>
  );
  ...
  return (
    <div ref={cardRef} style={cardStyle} className="fluid-service-card">
      ...
      <div style={contentStyle}>
        ...
        <div style={titleStyle}>
          {!isLoading && service?.name}
        </div>
        
        {/* Kategorie */}
        {(service?.category || isLoading) && (
        ...
        {/* Standort */}
        {(service?.location || isLoading) && (
          <div style={locationStyle}>
            {!isLoading && service?.location}
          </div>
        )}
...
export default FluidServiceCard;
