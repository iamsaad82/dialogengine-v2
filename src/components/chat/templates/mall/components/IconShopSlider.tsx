...
import IconShopCard from './IconShopCard';

interface IconShopSliderProps {
  title: string;
  shops: ShopData[];
...
/**
 * Eine optimierte Shop-Slider-Komponente mit Icons und stabilem Layout
 */
const IconShopSlider: React.FC<IconShopSliderProps> = ({
  title,
  shops,
  style = {},
  isStreaming = false
  ...
  useLayoutEffect(() => {
    ...
    const calculateVisibleItems = () => {
      ...
      const itemWidth = 280; // Standardbreite einer Karte
  ...
  const scrollPrev = useCallback(() => {
    ...
    if (sliderRef.current) {
      const itemWidth = 280 + 16; // Kartenbreite + Margin
  ...
  const containerStyle = useMemo<React.CSSProperties>(() => ({
    ...
    overflow: 'hidden',
    position: 'relative',
    height: '460px', // Feste Höhe für Stabilität
  ...
  return (
    <div ref={containerRef} style={containerStyle}>
      <h3 ref={titleRef} style={titleStyle}>{title}</h3>
...
export default React.memo(IconShopSlider);
