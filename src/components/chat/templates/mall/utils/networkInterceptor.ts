...
// Speichert die vollständigen Antworten für spätere Analyse
let completeResponses: { url: string; text: string; timestamp: number }[] = [];
...
global.fetch = async function(input: RequestInfo | URL, init?: RequestInit) {
  ...
  try {
    ...
    if (url.includes('api/v1/prediction') || url.includes('api/chat')) {
      ...
      // Speichere die vollständige Antwort
      ...
      logData('API-Antwort (vollständig)', text);
...
// Funktion zum Abrufen der vollständigen Antworten
export function getCompleteResponses() {
  return completeResponses;
}

// Funktion zum Löschen der vollständigen Antworten
export function clearCompleteResponses() {
  completeResponses = [];
}
