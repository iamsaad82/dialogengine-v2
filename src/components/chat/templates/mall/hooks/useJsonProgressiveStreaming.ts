...
export function useJsonProgressiveStreaming(
  ...
  const processJsonContent = useCallback(() => {
    // Auch wenn der Content gleich ist, verarbeiten wir ihn erneut, um progressives Rendering zu ermöglichen
    if (isProcessingRef.current || !content) {
      return;
    }

    isProcessingRef.current = true;
    // Wir setzen lastContentRef nicht, damit wir den Content bei jedem Update verarbeiten
  ...
  useEffect(() => {
    ...
    // Sofortige Verarbeitung ohne Debounce für besseres progressives Rendering
    processJsonContent();
