...
// VERSION-MARKER: Chat-Embed-Debug-Code - Version 007
console.log("Embed Chat Client - Debug-Version 007");
...
export default function ClientComponent() {
  ...
  useEffect(() => {
    ...
    console.log(`CHAT-EMBED-DEBUG-007: Bot-ID aus URL: ${id || 'nicht gefunden'}`)
    console.log(`CHAT-EMBED-DEBUG-007: Streaming: ${shouldUseStreaming ? 'aktiviert' : 'deaktiviert'}`)
    ...
    if (embedColor) {
      ...
      console.log(`CHAT-EMBED-DEBUG-007: Primärfarbe aus URL: ${embedColor}`)
    } else {
      console.log(`CHAT-EMBED-DEBUG-007: Keine Primärfarbe in URL, verwende Standard: ${primaryColor}`)
    }
    ...
    if (modeParam === 'bubble' || modeParam === 'inline' || modeParam === 'fullscreen') {
      setMode(modeParam)
      console.log(`CHAT-EMBED-DEBUG-007: Chat-Modus aus URL: ${modeParam}`)
    } else {
      console.log(`CHAT-EMBED-DEBUG-007: Kein gültiger Modus in URL, verwende Standard: ${mode}`)
    }
  ...
  useEffect(() => {
    const observer = new ResizeObserver(() => {
      ...
      console.log(`CHAT-EMBED-DEBUG-007: Sende Resize-Event: ${width}x${height}`)
    })
